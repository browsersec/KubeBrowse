...
import { WS_BASE, HTTP_TUNNEL_BASE } from "../config";

// Set custom Mouse implementation
Guacamole.Mouse = GuacMouse.mouse;

// Define websocket and HTTP tunnel URLs
const wsUrl = `${WS_BASE}/websocket-tunnel`;
const wsSharedUrl = `${WS_BASE}/websocket-tunnel/share`;
const httpUrl = `${HTTP_TUNNEL_BASE}/tunnel`;
...
function GuacClient({
  ...
  const resize = () => {
    ...
    if (displayRef.current) {
      ...
      displayRef.current.style.marginLeft =
        scaledWidth < viewportWidth ? `${(viewportWidth - scaledWidth) / 2}px` : "0";

      // Center vertically
      displayRef.current.style.marginTop =
        scaledHeight < viewportHeight ? `${(viewportHeight - scaledHeight) / 2}px` : "0";
    }
  };
  ...
  return (
    <div className="relative w-full h-full">
      <div
        className="fixed top-0 left-0 w-screen h-screen overflow-hidden bg-black flex justify-center items-center"
        ref={viewportRef}
      >
        <div
          className="guac-display outline-none w-auto h-auto mx-auto"
          ref={displayRef}
          tabIndex="0"
        >
