...
const OfficeSession = () => {
  ...
  const saveSessionToStorage = (sessionData: Partial<SessionState>) => {
    const sessionInfo = {
      ...sessionData,
      timestamp: Date.now(),
  ...
  const joinExistingSession = async (uuid: string) => {
    try {
  ...
  const handleConnectionStateChange = (state: string, attempts: number) => {
    setReconnectStatus({
