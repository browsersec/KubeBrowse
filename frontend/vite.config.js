        ...
        configure: (proxy, _options) => {
          ...
          proxy.on("error", (err, req, res) => {
            ...
            if (err.code === "EPIPE" || err.code === "ECONNRESET") {
              console.log(
                "Connection closed by remote host. Preventing further writes."
              );
              if (res && !res.headersSent) {
                res.writeHead(502, { "Content-Type": "text/plain" });
                res.end(
                  "WebSocket connection error. Please refresh the page to reconnect."
                );
              }
            }
          });
          ...
          proxy.on("econnreset", (err, req, res, target) => {
            ...
            if (res && !res.headersSent) {
              res.writeHead(502, { "Content-Type": "text/plain" });
              res.end(
                "Connection reset by server. Please refresh the page to reconnect."
              );
            }
          });
