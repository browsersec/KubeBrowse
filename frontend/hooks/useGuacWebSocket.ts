...
import { states, ConnectionState } from '../lib/guac/states';
...
interface UseGuacWebSocketOptions {
  wsUrl: string;
  httpUrl: string;
  forceHttp?: boolean;
  queryString?: string;
...
interface UseGuacWebSocketReturn {
  client: any | null;
  connectionState: string;
  errorMessage: string;
  readyState: number;
  ...
  sessionUUID: string | null;
  enableSharing: boolean;
  enableSessionSharing: () => Promise<boolean>;
...
const useGuacWebSocket = (
  ...
  enableSharing = false,
  onConnectionStateChange?: (state: string, attempts: number) => void
): UseGuacWebSocketReturn => {
  const [connectionState, setConnectionState] = useState<string>(states.IDLE);
  const [errorMessage, setErrorMessage] = useState('');
  const [isConnectionUnstable, setIsConnectionUnstable] = useState(false);
  const [sessionInfo, setSessionInfo] = useState<SessionInfo>({ userCount: 1, isShared: false });
  const [reconnectAttempts, setReconnectAttempts] = useState(0);
  const [isSessionOwner, setIsSessionOwner] = useState(false);

  const clientRef = useRef<any>(null);
  const tunnelRef = useRef<any>(null);
  const sessionConnectionRef = useRef<any>(null);
  ...
  }, [connectionState, reconnectAttempts, onConnectionStateChange]);
  ...
  const saveSessionToStorage = useCallback((sessionData: Partial<SessionStorage>) => {
  ...
  const loadSessionFromStorage = useCallback((): SessionStorage | null => {
  ...
  // Enhanced reconnection logic with session persistence
  ...
  }, [loadSessionFromStorage, queryString]);
  ...
  class ReactWebSocketTunnel extends Guacamole.WebSocketTunnel {
    receiveCallback: ((data: string) => void) | null = null;
    sessionConnection: any = null;
    reconnectOnClose: boolean = true;
    webSocketRef: any = null;
    ...
    /* outgoing traffic uses WebSocket */
    ...
    setWebSocketRef(ref: any) {
      this.webSocketRef = ref;
    }

    setReceiveCallback(callback: (data: string) => void) {
  ...
  useEffect(() => {
    ...
    tunnel.onstatechange = (state: any) => {
      switch (state) {
        ...
        case Guacamole.Tunnel.State.UNSTABLE:
          ...
          setTimeout(() => {
            if (tunnelRef.current && tunnelRef.current.currentState === Guacamole.Tunnel.State.UNSTABLE) {
  ...
  const enableSessionSharing = async (): Promise<boolean> => {
